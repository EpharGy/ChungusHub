...
| Per-chat image workflow | `feature/imagegen-per-chat` | One story generates with its own ComfyUI workflow instead of the app's, so a character can carry a LoRA the rest of them do not. Empty means follow the default. See `architecture/imagegen.md`. |
...
| Assistant in the title bar | `feature/assistant-titlebar` | A home for the Chungus Assistant in the title bar, for when its floating corner launcher is switched off. The switch sits on the assistant's own settings page, asked the way somebody arrives at it ("Show Chungus Assistant on top bar instead of floating icon"), and drives the same field as the existing Settings > General row rather than a second copy of it. Hiding the launcher used to leave Ctrl/Cmd+J as the only way into the panel; now a plain **Assistant** button stands beside the other title-bar toggles, opening and closing it and carrying the same running and waiting-for-approval marks the launcher shows, read from the same store getters so the two surfaces cannot disagree about whether the assistant is working. Written for a phone, where the corner launcher sits over the text being read and has to be dragged clear again every few turns. See `architecture/chungus-assistant.md`. |

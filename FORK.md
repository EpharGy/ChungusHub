...
| Image generation | `feature/comfy-inject` | `[[IMG: ...]]` markers in a reply become pictures, drawn by a ComfyUI server the backend dials directly. Settings page for host, workflow, sampler and framing. One story can also generate with its own workflow instead of the app's, so a character can carry a LoRA the rest of them do not; empty means follow the default. See `architecture/imagegen.md`. |
...
| Text pop-out | `feature/text-popout` | Any multi-line field opens in a large editor: focus it and a button appears in its corner. A panel on a desktop, the whole screen on a phone, in the field's own font, and every keystroke goes straight back into the field, so nothing needs saving on close. Works on fields inside dialogs, where Escape closes the editor and leaves the dialog open. The chat composer, in-place message editing and the Chungus Assistant's message box are left out; `data-no-popout` leaves out any other field. See `architecture/text-popout.md`. |
| Press-twice delete confirm | `feature/delete-confirm-gesture` | Settings → Advanced → Delete confirmations offers "Press twice" beside "Press and hold" while deletes are on Ask and hold. A big delete then arms on the first press ("Press again to confirm") and fires on the second; a second press within 300ms is ignored, and the arm lapses after 3 seconds. For pens and touchscreens that turn a long press into a right click and cancel the hold. Which deletes need the heavy confirm is unchanged. See `architecture/ui-shell-settings.md`. |
...
There are none carried right now. All three that were here are in upstream `main` as of
September 2026: two were merged from this fork, and the third was written upstream
independently while our own patch was still open. The fork runs upstream's copy of each.

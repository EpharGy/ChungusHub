...
| Gallery window | `feature/image-popout` | Keeps one picture from any card's gallery on screen while you work elsewhere in the app. Raised from the title bar, and it browses for its own image: a grid of every persona and character that has gallery pictures, then that card's pictures. Drag it anywhere, resize it, or drop it against an edge or corner to dock it; two header buttons page through the rest of that card's gallery. What is pinned is remembered per chat, so returning to a story brings its picture back, and hiding the window keeps the picture rather than throwing it away. On a phone it is a full-screen panel. See `architecture/image-popout.md`. |

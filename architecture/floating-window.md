...
**EchoChamber used to be the second entry here**, keeping its own copy of the same maths on a reason that had expired: the comment at the top of that widget said extracting a shared shell would have meant rewriting the Assistant, and this branch extracted one without touching it. It is a consumer now, so the paragraph is kept only to say that the exception it described is gone, and that "the Assistant is a 946-line upstream file" is the whole of what is left holding the one above.

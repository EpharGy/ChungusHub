...
Records for *other* chats that sourced the same deleted entry are deliberately left alone. Each of those chats can still be walked back into, and `restoreFor` reads the source's gallery live, misses, and tells that reader at the moment it is worth hearing: once, dropping the pin as it goes. Sweeping them in advance would take the explanation away and leave a window's absence unaccounted for.
...
## The two facts have two different homes

The picture and the frame are remembered in two different places, and which fact goes where is the one decision in this feature that has been wrong in production:

| Fact | Home | Why |
|---|---|---|
| The pinned picture | the chat row, `feature_state.galleryPin` | A reference pinned to a story is a fact about that story. It has to survive a cleared browser and be there on the phone. |
| Whether the frame is up | localStorage, [`popout-memory.ts`](../src/lib/utils/popout-memory.ts) | Means nothing on another screen, and a window left standing on a desktop would take a phone's whole screen the moment that story opened. |
| Where the frame is | localStorage, via `FloatingWindow`'s `storageKey` | A rectangle means nothing on another machine. |

**Both used to be in the localStorage record**, one object per chat holding the path, its source and the open flag together, on the reasoning that a window's state is per-device. That is true of the frame and false of the picture. A picture pinned on the desktop was invisible from the phone and the other way round, and it failed *silently*: a device with no record restores nothing, so there is no miss to report and nothing on screen to notice. The notepad had already made this split correctly, one branch over, citing this feature as its model for the half it kept local.

So the record here is now a bare list of chat ids, and it is very nearly `notepad-memory.ts`. Everything the old shape needed to bound a per-chat map of pictures went with the pictures: a chat-row field is reaped by the chat delete, on every device at once, like the notes and EchoChamber's feeds.

**Only the path is stored, never the set.** It is re-read from the source entry's live gallery on the way in, which turns a picture deleted in the meantime into a miss that can be reported (a toast, and the pin dropped so the notice comes once) rather than a broken image rendered out of a stale snapshot. Same reasoning as the snapshot's missing-file state, arriving at the opposite answer because a reopen has somewhere live to look and a paging window does not.

**The pin is named for the pin, not for the feature**, unlike `notepad` and `echoChamber` beside it on the row. A field called `imagePopout` would invite the next reader to put the open flag in it, which is exactly the mistake this shape exists after.

**Writes to the row are debounced**, at the scene store's 250ms rather than the notepad's 600. A chat row write broadcasts the `chats` scope and every other device answers with a chat-list refetch, and paging is a burst of clicks: a refetch per arrow press is the shape that would make. Paging is a burst that *ends*, like a slider drag, which is why it needs a quarter of a second and typing needs more than twice that. The debounce is flushed on a chat switch and on the way off the page, each carrying its own chat id, so a page turned up to the instant of a switch lands on the story it was made in.

The open-chats list is capped at twenty, most-recent-first. The cap is what makes it a **bound**: it is enforced on write, by a file that is read before any store loads, so it holds no matter what else is or is not working. The cap alone still leaves it stale, so an effect **prunes** it against the live chat list, which the window can do because it is mounted for the app's whole life. Driving it off the list rather than out of the delete paths means one row, a batch, and a delete arriving from another device are all the same event, no delete path has to know this feature exists, and (the reason it matters for a fork) `chat.svelte.ts` is not touched at all. It writes only when something actually goes, so the ordinary case costs a read.

**Two dead localStorage keys are swept on read, and neither is migrated.** `image-popout-by-character` predates the chat binding: turning one of those into a chat-keyed record means guessing which of that character's chats the picture was pinned up in, and guessing wrong puts a window in a story nobody opened it in. `image-popout-by-chat` is the one that wrongly held the picture, and promoting its paths onto the rows they belong to would have to run inside each loaded chat, landing one story at a time over days, with no way to tell when it was finished and therefore no moment the migration code could be removed. Dropping costs each reader one re-pin per story they had one in. Keeping either would park a dead key in every browser forever.
...
Half of that was answered by the registry, and half by the shell growing a full-screen mobile panel. One more thing was answered later, and it is the reason a phone gets anything worth having: the pinned picture lives on the chat row, so a reference pinned up at the desk is there on the phone, in that story, without being pinned again. While it was in localStorage the phone got a panel it could open and nothing to open it on. The remaining objection was that a full-screen picture on a phone looks like the viewer it was opened from, and it is worth saying why that is not an argument: the viewer closes with the library, and this window stays up while the reader goes back to the story. Outliving the surface that opened it is the entire feature, and a phone is where the library covers everything, so it is worth more there rather than less.

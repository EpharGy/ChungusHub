...
**Where it runs is a deliberate short list.** After a picture lands, because that is the only event that grows the cache; at boot via `runBootSweeps`; and from the button in Settings → App → Image Generation. Chat load and message send were both considered and are both wrong: neither adds a picture, so both would spend the walk to be told nothing changed, on the two paths where the reader is most obviously waiting.

**The boot pass exists because of the grace window, not in spite of it.** A restart takes nothing away - the window is a function of each picture's own stored `createdAt`, so it expires on schedule whatever the process did, and a picture that was 50 minutes old across a restart is simply sweepable ten minutes later. What a restart *does* remove is the next occasion to ask: with generation as the only other automatic trigger, a session that ended over budget would sit there until the reader next made a picture, which could be days. `sweepGeneratedImageCacheToBudget` reads the budget out of the `imagegen` settings row itself, and every way of failing to understand that row - missing, unparseable, a build that predates the field, a value that is not a positive number - answers "no budget" and deletes nothing.

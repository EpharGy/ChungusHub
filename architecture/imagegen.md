...
**A boot pass was written and then removed on purpose**, so here is why, before someone adds it back. The grace window needs no help surviving a restart: it is a function of each picture's own stored `createdAt`, so it expires on schedule whatever the process did, and a picture 50 minutes old across a restart is sweepable ten minutes later. The only thing a restart removes is the next occasion to ask - and the next picture is that occasion. A session that ends over budget is over budget until the reader generates again, which is the same moment they would care. Note the trigger is a picture landing rather than a message being sent, so a chat that stops using markers stops sweeping; the button is the answer to that, and it is a click rather than a startup cost paid on every boot forever.
...
`ChatFeatureState.imagegenWorkflow` is a filename or null, and null - the normal case - follows the app's setting. It exists for one want: a character with a LoRA of their own, in the story that character is in and in no other. Without it the app-wide workflow is swapped by hand on every chat switch, which is a setting nobody remembers to put back.

It rides `ChatFeatureState` rather than a table or a column of its own, so it inherits that blob's whole lifecycle: deleting the chat deletes it, duplicating the chat copies it, and the `chats` sync scope already broadcasts it. No DDL, no migration, no reaping. A blob written before this existed simply has no key, and a missing key is "follow the app" - the same reason `connection`, `persona` and `preset` needed no migration either.

**The claim is read in the same synchronous prefix as the seed** (`generate`, imagegen.svelte.ts), before the first await, and for the same reason: both answer "which story is this marker in". A reader who walks to another chat while a picture is being made must not have that chat's workflow swapped under a job already in flight. Asking the OPEN chat is correct rather than convenient, because generation is already bounded to it on both sides - `ensureForMessage` resolves its row through the open chat and a row outside it resolves to nothing, and `resolveSeed` walks that chat's own message list.

**`buildGenerateRequest` takes the resolved name as an argument** and looks nothing up. `request.ts` is free of `$lib` and of Svelte so the whole of it is unit-testable with no DOM and no store, which is the same contract `seed` already arrives under.

**A claim naming a workflow that is no longer there fails the generation, and must keep failing it.** Falling back to the app's would be the friendlier-looking answer and it is the wrong one: the claim was made to get a specific LoRA, so the fallback is a picture without it, correct in every visible way except the one the reader cared about, with nothing on screen saying so. `readWorkflow` throws naming the file, the same as a missing checkpoint does, and the picker marks the name `(missing)` exactly as the app-wide one already does.

**Nothing sanitizes the name on the way in, deliberately.** A name is only dangerous where it becomes a path, and `safeWorkflowName` already rejects anything that is not a bare `.json` basename - rejects rather than cleans, because a name that needed cleaning was not the file anyone meant to run. A second, quieter check in the normalizer would put a different answer in front of that one.

**Empty means "follow the default", and is stored as null rather than as the app's current name.** Writing the name would pin the story to whatever the default happens to be today and silently stop it following, which looks identical on the settings page until the day the default is changed for every other chat.

**The per-chat picker is not hidden when no chat is open**, it is disabled and says why. It sits directly under the app-wide Workflow field on the same page, which is what makes "default unless this story says otherwise" readable without a sentence explaining it.

## Where it plugs in

- `messageStore.triggerImageGeneration(messageId)` - fire-and-forget after a reply, a continuation and an opening scene, beside `triggerMemoryMaintenance`. Each caller names the row it just wrote. Named rather than scanned for: a scan of the open path answers "whatever turn is newest right now", which is the turn that just landed only while the reader stays put, so walking to another branch or another chat mid-generation would hang one turn's markers on another turn's row. A row that is not in the open chat resolves to nothing, which is why the next bullet exists.
- `imagegenStore.ensureForNewestReply()` - **the second chance, and the only thing that re-asks.** The trigger above fires once, from the generation that placed the row, and resolves that row through the open chat; but a reply is committed by the *server*, so it lands whether or not the reader is still on that chat. Switch character mid-reply and its markers were passed over silently, with nothing to notice and no way back but the buttons. `ChatContainer` now asks on every change and the store decides whether that means a call, exactly as `SpriteLayer` calls `spriteStore.ensureRead` (architecture/engines.md). **Bounded to the newest reply on the open branch, never a sweep back through it**: opening an old chat must not queue a pile of GPU jobs. Every other guard is `ensureForMessage`'s own, so a picture already made, one being made, and one that failed are all left alone. `newestReply` is a plain getter rather than a `$derived` - it is read once per pass, so a cache would buy nothing and would cost this store its first rune beyond `$state`, which the tests shim.
...
- **The turn is claimed synchronously, on the last line before the first await** (`passes`). This is the Sprites engine's `inFlight` rule, and leaving it out is what made the whole design misfire once upstream started moving the transcript's state more often. `ensureForNewestReply` is called from a `$effect`, and the *synchronous prefix* of `ensureForMessage` reads `working`, `failures` and the row's attachments - so claiming a marker re-runs the very effect that asked. The interrupting pass then finds every marker the first has not reached yet still unclaimed, because `working` is set per marker inside `generate`, several awaits later. Two passes over a five-marker turn is not the bad case; it compounds, one fresh pass per claim. **What the reader sees is not a duplicate picture but a missing one**: the re-ask is byte-identical, prompt and seed included, so ComfyUI answers it from its own cache in milliseconds and files a history entry carrying no output, which `waitForImage` polls until it gives up - the marker spins for the full timeout and the toast blames a host that did nothing wrong. A plain `Set`, not a `SvelteSet`: nothing on screen reports it, and making it reactive would wake that effect once more per turn to be told the same thing.
- **The per-marker loop asks `working` again before each picture**, which the list it is walking asked a minute ago. The pass claim keeps other passes out; this is what keeps the Generate button on a marker from being duplicated by the pass walking towards it.
...
- Settings ride the `settings` spine under `imagegen`, and the page is Settings → App → Image Generation. The one per-chat setting is not on that spine at all: see **One workflow per story** above. It is deliberately **not** an entry in the Engines registry: every engine there is a routing point for an LLM connection, and this one answers to a diffusion server (architecture/engines.md).
...
- `bun test src/lib/stores/imagegen-preflight.test.ts` covers the half that is not pure: how a turn sequences its calls when the host is not there. It shims `$state` and mocks the five modules the store reaches for, following `new-chat-flow.test.ts`. What it pins is a count - one ping, one generation attempt - so deleting either rule turns it red rather than leaving a comment nobody reads. It also pins the **re-entrancy** count, with a second pass entered both before the first has claimed anything and in the middle of its first picture; without the claim those read six and five generations against three, which is the duplicate-seed signature in ComfyUI's output folder.
- `bun test src/lib/stores/imagegen-arrival.test.ts` covers the other half that is not pure: what a chat still owes when the reader walks back into it. The rule worth guarding there is the **bound** - an older turn's markers on the same branch must stay unasked-for, because the cost of getting that wrong is GPU time, not a wrong pixel.
...
- A workflow name from the page is a filename and nothing else (`safeWorkflowName`): it is joined onto a path and read off disk. A name from a chat's own claim reaches the same guard by the same route, which is why the claim's normalizer does not clean it.
- **The checkpoint list is probed when the settings page opens, quietly**, and the field that shows the stored name while the host is unreachable is read-only. A checkpoint is a filename the host has to recognise exactly, so a typed one has no feedback until a generation fails deep inside a workflow - and there is no lockout in refusing the keyboard, because the list is fetched by the server over the same connection a generation uses. The button keeps its loud mode, with the toast, for a host that was changed or was down a moment ago; only the probe on open is silent, because a settings page that raises a toast when the GPU box is asleep is noise the status line already covers.

...
The notepad is a thing you return to, so it gets a title bar entry, beside Preset Controls, Story Map and Memory, which is where the reader already looks for what belongs to the story they are in. Not a draggable button at the workspace edge: the row already exists, and a launcher pinned to the edge of the screen is a permanent third thing to look at.
...
- **The row's label-width threshold covers this button**, but not because this feature raised it. `TitleBar` computes `NAV_LABELS_MIN_REM` from `TOPBAR_INLINE_LIMIT`, so the allowance for a panel label is already there before any panel registers, and adding one costs the rule nothing.
...
## It works on a phone, inherited rather than chosen

`FloatingWindow` is a full-screen panel docked to the workspace on mobile, so the notepad is one there too: same notes, same header, no dragging. Nothing in this feature says so, and the chat-switch effect below runs at every width.

It did not always. The shell used to render nothing on a phone, so the title bar filtered the button out and `followChat` skipped, on the reasoning that a window with no launcher is state nobody can see or close. The registry removed the launcher problem, the shell grew a mobile branch, and both gates went with it. Worth knowing because the notes were always the half that belonged on a phone: they live on the chat row precisely so they survive a cleared browser and are there on the other device.

...
<script lang="ts">
	...
	// The note is asked in the composer, never in this menu: a field in the transcript ends up under
	// a phone's keyboard (architecture/engines.md). The star leaves from Retry, tying menu to box.
	function handleRewriteClick() {
		showRegenerateMenu = false;
		if (message.parentId) composerQuestion.open({ kind: 'rewrite', parentId: message.parentId }, regenerateButton);
	}
									...
									>
										...
										<div class="p-1.5">
											...
											{#if !isUser}
												...
												>
													<span class="font-medium text-text-primary">Rewrite with a note</span>
													<p class="text-text-muted text-xs mt-0.5">
														Say what to change and the AI rewrites this response
													</p>
												</button>
											{/if}
											<button
										...
										<button
											...
											onclick={() =>
												composerQuestion.asking('opening')
													...
													: composerQuestion.open({ kind: 'opening' }, openingButton)}
											disabled={messageStore.isStreaming}
											aria-pressed={composerQuestion.asking('opening')}
											aria-label="Write another opening scene"
											title={composerQuestion.asking('opening') ? 'Back to your message' : 'Write another opening scene'}
										>
